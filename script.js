...
// Winner deciding Screen
const winnerDecider = function () {
  header.textContent = `Congratulations! ${winner} is the winner. 🥇🎉`;
  header.style.fontSize = '40px';
  header.style.color = '#fff';
  playAgainButton.textContent = 'PLAY AGAIN↩️';
  playAgainButton.classList.add('btn', 'btn--new');
  document.body.append(header);
  document.body.appendChild(playAgainButton);
  header.classList.remove('game');
  playAgainButton.classList.remove('game');
  playAgainButton.style.marginTop = '200px';
};
...
const playAgain = function () {
  init();
  header.classList.add('game');
  playAgainButton.classList.add('game');
  console.log('clicked');
};
